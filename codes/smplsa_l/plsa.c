...
void iniStructures(Mat *pwd, int Z, char *init_pzd_hpc, char *init_pwz_hpc){
	...
	// Inicializamos PZD segun proceda
	if(strcmp(init_pzd_hpc,"rand")==0) // random (filas normaizadas a partir de numeros aleatorios entre 0 y 1000)
		pzd = createMat(D,Z,10000);
	else if(strcmp(init_pzd_hpc,"unif")==0) // uniforme (para intentar evitar "picos" en pwz)
		pzd = createMat(D,Z,-1);
	else // cargamos la inicialización de pwz de un fichero de texto
		pzd = loadMat(init_pzd_hpc,0,D,Z);
	
	// Inicializamos PWZ segun proceda
	if(strcmp(init_pwz_hpc,"rand")==0) // random (filas normaizadas a partir de numeros aleatorios entre 0 y 1000)
		pwz = createMat(Z,W,10000);
	else if(strcmp(init_pwz_hpc,"unif")==0) // uniforme (para intentar evitar "picos" en pwz)
		pwz = createMat(Z,W,-1);
	else // cargamos la inicialización de pwz de un fichero de texto
		pwz = loadMat(init_pwz_hpc,0,Z,W);
...
void Estep(){
	...
	int W=pzwd->size[0]; // numero de palabras
  
	// TBB parallelisation
	parallel_for_(Range(0, W), Parallel_Estep_pzwd(pwz,pzd,pzwd));
}
...
void Mstep(Mat *pwd, double lambda_pzd, double lambda_pwz){
	...
	int D=pzwd->size[1], Z=pzwd->size[2]; // numero de documentos y topicos

	// TBB parallelisation
	parallel_for_(Range(0, Z), Parallel_Mstep_pwz(pwd,pzwd,lambda_pwz,pwz));
	parallel_for_(Range(0, D), Parallel_Mstep_pzd(pwd,pzwd,lambda_pzd,pzd));
}
...
double logL(Mat *pwd){
	...
	// TODO: parallelise this loop with TBB
	for(int d=0;d<D;d++){
		for(int w=0;w<W;w++){
			double sumz=0.0;
			for(int z=0;z<Z;z++)
				sumz += pwz->at<double>(z,w) * pzd->at<double>(d,z);
			sumz = pd->at<double>(d) * sumz;
			if(sumz>0.0) // para evitar calcular el log de un valor negativo
				L += pwd->at<double>(d,w) * log(sumz);
		}
	}
...
void plsaEM(Mat *pwd, int Z, double lambda_pzd, double lambda_pwz, int fix_iters, char *init_pzd_hpc, char *init_pwz_hpc){
	...
	iniStructures(pwd, Z, init_pzd_hpc, init_pwz_hpc);
	...
	for(int it=0; it<NUM_ITERS; it++){
		...
		Mstep(pwd,lambda_pzd,lambda_pwz);
		if(fix_iters==0){ // regular mode (1000 EM iters & 1e-6 threshold)
